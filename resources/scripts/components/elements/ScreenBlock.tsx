    ...
    <PageContentBlock>
        <div css={tw`flex justify-center`}>
            ...
            >
                ...
                {/* Si 'image' es un componente JSX, se renderizará aquí */}
                {typeof image === 'string' ? (
                    <img src={image} css={tw`w-2/3 h-auto select-none mx-auto`} />
                ) : (
                    <div css={tw`w-2/3 h-auto select-none mx-auto`}>{image}</div> // Renderizar JSX si es pasado
                )}
                <h2 css={tw`mt-10 text-neutral-200 font-bold text-4xl`}>{title}</h2>
                <p css={tw`text-sm text-white mt-2`}>{message}</p>
            </div>
        </div>
    </PageContentBlock>
);
...
const ServerError = ({ title, ...props }: ServerErrorProps) => (
    <ScreenBlock title={title || 'Algo salio mal'} image={ServerErrorSvg} {...props} />
);
...
const NotFound = ({ title, message, onBack }: Partial<Pick<ScreenBlockProps, 'title' | 'message' | 'onBack'>>) => (
    <ScreenBlock
        title={title || '404'}
        image={NotFoundSvg}
        message={message || 'La pagina solicitada no existe.'}
        onBack={onBack}
    />
);
